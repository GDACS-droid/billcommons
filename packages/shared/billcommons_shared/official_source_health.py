...
def collect_official_source_health(db, *, jurisdiction_ids: tuple, now: datetime) -> dict[Any, tuple[OfficialTargetHealth, ...]]:
    ...
    rows = db.execute(statement, {"jurisdiction_ids": jurisdiction_ids,
                                  "row_limit": MAX_OFFICIAL_HEALTH_TARGETS + 1}).mappings().all()
