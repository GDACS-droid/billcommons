...
from billcommons_shared.data_health import collect_report, exit_code
...
def _official_target(db_session, jurisdiction):
    target = OfficialSourceTarget(
        jurisdiction_id=jurisdiction.id, adapter_name="source-health-test/1",
        source_url=f"https://example.invalid/{uuid.uuid4()}", enabled=True,
        cadence_seconds=3600, next_check_at=NOW + timedelta(hours=1),
    )
...
def test_latest_official_observation_is_scoped_and_deterministic(db_session):
    ...
    def observation(identity, status, retrieved, created):
        return OfficialSourceObservation(
            id=uuid.UUID(int=identity), target_id=target.id,
            adapter_name=target.adapter_name, adapter_version="test/1", source_url=target.source_url,
            retrieved_at=retrieved, created_at=created, status=status,
    ...
    db_session.add_all([
        ...
        observation(4, "succeeded", NOW, NOW - timedelta(seconds=1)), failure,
    ])
    db_session.flush()
    ...
    future = observation(6, "succeeded", NOW + timedelta(minutes=6), NOW)
    future.source_url = latest.source_url
    db_session.add(future)
    db_session.flush()
    report = collect_report(db_session, now=NOW)
    source = _report_row(report, "CA")["source_health"]["official_sources"]
    assert source["targets_by_state"]["future_observation"] == 1
    defect = next(d for d in report["defects"] if d["jurisdiction"] == "CA"
                  and d["code"] == "OFFICIAL_SOURCE_FUTURE_OBSERVATION")
    assert defect["severity"] == "error" and exit_code(report, "error") == 1
