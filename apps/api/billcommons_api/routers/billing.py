    ...
    return (
        db.execute(
            select(ApiKey.id).where(
                ApiKey.customer_id == customer_id, ApiKey.status.in_(("active", "rotating"))
            )
        ).first()
        is not None
    )
...
def _apply_subscription_event(
    ...
    if row is not None and row.last_event_created_at is not None:
        ...
        # Item 19: Stripe's `created` has one-second resolution, so an
        # `updated` and the `deleted` that immediately follows it can
        # legitimately share a timestamp. `>=` (the apply condition) can't
        # tell them apart by ordering alone -- break the tie by preferring
        # a TERMINAL transition: once this row is terminal, a same-second
        # non-terminal event never revives it.
        #
...
def _handle_checkout_session_completed(
    ...
    if sub_id:
        ...
        if row is None:
            ...
            outcome = _apply_subscription_event(db, customer, remote_sub, event_created, background_tasks)
            row = db.execute(
                select(ApiSubscription).where(ApiSubscription.stripe_subscription_id == sub_id)
            ).scalar_one_or_none()
    ...
    _ensure_provisioned_for_subscription(db, customer, row, background_tasks, send_magic_link=False)
...
def _handle_charge_refunded(
    ...
    if cancel_access:
        row = _active_subscription(db, customer.id)
        if row is not None:
            _configure_stripe()
...
def stripe_webhook(
    ...
    inserted = db.execute(
        text(
            "INSERT INTO stripe_events (id, type, payload) VALUES (:id, :type, :payload) "
