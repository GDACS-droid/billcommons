...
const ENDPOINTS: { method: string; path: string; description: string }[] = [
  ...
  { method: "GET", path: "/api/v1/bills", description: "List bills. Filters: jurisdiction, session, identifier, chamber, status, subject, sponsor. (No committee or date-range filter — those were listed here before they existed.)" },
  ...
  { method: "GET", path: "/api/v1/people", description: "NOT COLLECTED — always returns []. Legislator records are not ingested; responses carry data_status: \"not_collected\". Sponsor names are available per bill." },
  { method: "GET", path: "/api/v1/people/{id}", description: "NOT COLLECTED — always 404s. No legislator records exist." },
  { method: "GET", path: "/api/v1/committees", description: "NOT COLLECTED — always returns []. Committee referrals are visible in each bill's action history." },
  { method: "GET", path: "/api/v1/committees/{id}", description: "NOT COLLECTED — always 404s." },
  { method: "GET", path: "/api/v1/events", description: "NOT COLLECTED — always returns []. No hearing or calendar data is ingested for any jurisdiction. Empty means we lack the data, not that nothing is scheduled." },
  { method: "POST", path: "/api/v1/alerts/subscribe", description: "Email digest for a topic. Optional jurisdiction (e.g. \"FL\") scopes it to one state instead of all 51." },
