...
`BILLCOMMONS_SCOUT_ENABLED` gates API creation and web navigation. A separate worker command consumes only Scout jobs; it never holds a database transaction across network/browser I/O. Claims have a bounded lease and heartbeat; expired running jobs are reclaimable, and retries idempotently skip already-persisted source hashes. Cancellation is a versioned durable state checked between every source/action and triggers bounded browser release.

The route can be deployed dark, migration first, then API/worker, then web flag. Rollback disables creation and new claims, releases known live provider sessions, and marks lease-expired running jobs `failed` with `rolled_back`, while preserving completed/partial results for owner reads.
